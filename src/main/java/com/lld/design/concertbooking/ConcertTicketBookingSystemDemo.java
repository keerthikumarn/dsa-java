...
public class ConcertTicketBookingSystemDemo {
	public static void run() {
		...
		List<Concert> searchResults = bookingSystem.searchConcerts("Artist 1", "Venue 1",
	...
	private static List<Seat> selectSeats(Concert concert, int numberOfSeats) {
		...
		List<Seat> availableSeats = concert.getSeats().stream().filter(seat -> seat.getStatus() == SeatStatus.AVAILABLE)
